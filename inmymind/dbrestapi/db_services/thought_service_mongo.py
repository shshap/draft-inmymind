...
from mongoengine.errors import NotUniqueError, ValidationError, OperationError, FieldDoesNotExist
from typing import List, Optional

from inmymind.dbrestapi.db_services._objects_mongo import User, Snapshot, Pose, Feelings, Image

logger = logging.getLogger(__name__)

DEFAULT_DB_NAME_MONGO = 'inmymind'
DEFAULT_ALIAS_MONGO = 'core'
DEFAULT_HOST_MONGO = 'localhost'
DEFAULT_PORT_MONGO = 27017


class DbService_Mongo:

    def __init__(self, raw_url):
        url = urlparse(raw_url)
        db_name = url.path.replace('/', '') if url.path != '' else DEFAULT_DB_NAME_MONGO
        host = url.hostname if url.hostname else DEFAULT_HOST_MONGO
        port = url.port if url.port else DEFAULT_PORT_MONGO
        dict_queries = {attr.split('=')[0]: attr.split('=')[1] for attr in url.query.split('&')}
        alias = dict_queries.get('alias', DEFAULT_ALIAS_MONGO)
        connect(db_name, alias=alias, host=host, port=port)
